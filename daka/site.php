...
class Hai105_dakaModuleSite extends WeModuleSite {
    ...
    public function __construct() {
        ...
        if(strpos($this->config['blacklist'],$_W['openid'])!==false) {
            header("Location: ".$this->config['blackurl']);
            exit;
        }
    ...
    function __call($name, $arguments) {
		...
		if($isWeb || $isMobile) {
			if($isWeb) {
                ...
                $file = MODULE_ROOT."/inc/web/{$controller}.php";
			}
			if($isMobile) {
                $controller = strtolower(substr($name, 8));
                $file = MODULE_ROOT."/inc/mobile/{$controller}.php";
            }
			if(file_exists($file)) {
				require $file;
				exit;
    ...
    public function payResult($params) {
        if ($params['result'] == 'success' && $params['from'] == 'notify') {
            ...
            if($this->config['pay_list']){
                $paylist = json_decode($this->config['pay_list'],true);
                foreach($paylist as $k=>$v) {
                    if($order['pay_money']==round($v['value'],2)){
                        $go = true;
                        break;
                    }
                }
            }else{
                if($order['pay_money']==round($this->config['per_money'],2)) {
                    $go = true;
                }
            }
            if(!$go) {
...
//            message('请在微信端打开');exit;
        ...
        if(date('H:i')>$this->config['daka_end_time']) {
            $w = array(
                'uid' => $_W['member']['uid'],
            ...
            $myorder = pdo_get('hai_daka_yiyuan',$w);
        }
...
//        }
        ...
        if($res) {
            ...
            unset($today['id']);
            unset($today['checked']);
            unset($today['checked_time']);
            $today['create_date'] = date('Y-m-d');
            $today['create_time'] = date('Y-m-d H:i:s');
            $today['order_sn'] = 'h'.date("ymdHi").sprintf("%06d", $today['uid']);
            pdo_insert('hai_daka_yiyuan',$today);
            returnJson(0,'打卡成功');
        }else {
...
//            ));
    ...
    protected function _pay ($order,$money,$payment,$certurl,$isQiye=false) {
        ...
        if($isQiye){
            ...
            //处理提现用企业付款
            if($this->config['atmtype']==1) {
                return qiyeLuckyMoney($data,$payment,$certurl);
            }elseif($this->config['atmtype']==2){
    ...
    protected function cron () {
        ...
        if($this->config['cronkey']) {
            if($_GPC['key']!=$this->config['cronkey']){
                exit('error');
            }
        }
        ...
        $allMoney = $failpaymoney*(1-$this->config['tax_bili']);

        $zanzhu = pdo_get('hai_daka_zanzhu',array(
            'uniacid' => $_W['uniacid'],
            'date' => date('Y-m-d',strtotime('-'.($dd-1).' day'))
        ));
        ...
        foreach ($successList as $member) {
            ...
            if($member['pay_money']<1){
                continue;
            }
            $per = $member['pay_money']*($allMoney/$successpaymoney);
            $per = round($per,2);
    ...
    protected function _addCron () {
        ...
        $cron = array(
            ...
            'type' => 1,
            'name' => 'pay',
            'filename' => $this->config['cronkey'] ? $this->config['cronkey'] : 'pay' ,
            'nextruntime' => $next,
            'lastruntime' => $next,
            'status' => 1,
    ...
    public function doWebList () {
        global $_W,$_GPC;
        $_W['page']['title'] = '参与人员';
       /*  if($_W['isajax'] && $_GPC['op']=='buqian') {
            $id = $_GPC['id'];
            $order = pdo_get('hai_daka_yiyuan',array('id'=>$id));
            if(!$order || $order['status']!=1) {
                returnJson(1,'error');
            }
            if($order['checked']==1) {
                returnJson(1,'已经是打卡状态');
            }
            if($order['create_date']!=date('Y-m-d',strtotime('-1 day'))) {
                returnJson(1,'只能补最近一天');
            }
            $save = array(
                'checked' => 1,
                'checked_time' => date('Y-m-d').' 06:45:21'
            );
            $res = pdo_update('hai_daka_yiyuan',$save,array('id'=>$id));
            if($res) {
                returnJson(0,'补签成功');
            }else{
                returnJson(1,'补签失败');
        ...
        $where = array(
            'create_date' => $_GPC['date']?$_GPC['date']:date('Y-m-d'),
            'uniacid' => $_W['uniacid'],
            'status' => 1
        );
...
//            message('请在微信端打开');exit;
    ...
    protected function myinit () {
        ...
        $res['money'] = $this->member['credit2'];


        $res['myorder'] = $this->member['credit2'];
        returnJson(0, '',$res);
    ...
    protected function getdatelist() {
        ...
        if(!empty($list)) {
            foreach ($list as $k=>$v) {
                $v['create_date'] = date('Y-n-j', strtotime($v['create_date'])+24*3600);
                if($v['money']>0) {
    ...
    public function doWebConfig () {
        ...
        if(!$_GPC['op']||$_GPC['op']=='index'){
            ...
            if($_W['isajax']){
                $configlist = array(
                    'blacklist' => $_GPC['blacklist'],
                    'blackurl' => $_GPC['blackurl'],
                );
                foreach ($configlist as $k=>$v) {
                    pdo_update('hai_daka_config',array(
                        'value' => $v
    ...
    public function doWebAtm() {
        ...
        if($_W['isajax']) {
            ...
            if($action=='atm') {
                ...
                pdo_delete('hai_daka_atm',array('id'=>$id));
                returnJson(0, '已删除');
            }
        }
