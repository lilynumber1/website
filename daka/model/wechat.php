...
function refundOrder($log,$payment,$certurl){
    global $_W;
    $back['appid'] = $_W['uniaccount']['key'];//公众账号ID
    $back['mch_id'] = $payment['wechat']['mchid'];//商户号
    $back['nonce_str'] = generateNonceStr();//随机字符串
    $back['op_user_id'] = $payment['wechat']['mchid'];//商户号
    $back['out_trade_no'] = $log['uniontid'];//内部订单号
    //$back['transaction_id'] = $order['transaction_id'];//微信支付订单号
    $back['out_refund_no'] = $log['uniontid']."_1";//退款单号
    $back['refund_fee'] = intval($log['fee']*100);//单位为分
    $back['total_fee'] = intval($log['fee']*100);//订单总金额
    $back['refund_account'] = 'REFUND_SOURCE_RECHARGE_FUNDS';
    $back['sign'] = _getSign($back, $payment['wechat']['apikey']);//签名
    $xml = arrayToXml($back);
    $ext = array(CURLOPT_SSLCERT => $certurl);
    load()->func('communication');
    $result = ihttp_request('https://api.mch.weixin.qq.com/secapi/pay/refund', $xml, $ext);
